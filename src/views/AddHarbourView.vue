  ...
  <div class="container p-4 glass-background">
    ...
    <div class="container">
      ...
      <div>
        ...
        <div class="input-group mb-3">
          <select v-model="selectedCountyName" class="form-select mb-3"
        ...
        <div class="input-group mb-3">
          ...
          <select v-model="selectedCaptainContactId" class="form-select" aria-label="Default select example">
            <option selected :value="0">Vali kapten</option>
            <option v-for="captain in captainInfo" :value="captain.contactId" :key="captain.contactId">
          ...
          <div class="row">
            ...
            <div class="col">
              <button @click="openAddExtrasModal" type="button"
                      class="btn btn-secondary text-light fw-bold shadow-sm rounded-0 ">Lisa teenused
              </button>
            </div>
            <div class="col">
              <button @click="addNewHarbour" type="button"
                      class="btn btn-success text-light fw-bold shadow-sm rounded-0">Lisa sadam
              </button>
            </div>
            <div class="col">
              <button @click="moveToHarboursPage" type="button"
                      class="btn btn-primary text-light fw-bold shadow-sm rounded-0">Tagasi sadamate lehele
              </button>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    mandatoryFieldsAreFilled() {
      ...
      return info.harbourName.length > 0 && info.locationAddress.length > 0 && info.minWidth > 0 &&
          info.minDepth > 0 && info.spots > 0 && info.phoneNumber.length > 0 &&
          info.locationCountyName !== 'Kõik maakonnad' && info.contactId !== 0
    },

    sendAddNewHarbourRequest() {
      this.$http.post("/harbour", this.harbourDetailedInfo
      ).then(response => {
        this.successMessage = HARBOUR_LOCATION_ADDED.replace("?", this.harbourDetailedInfo.harbourName)
        setTimeout(() => { this.resetAllFields()
        }, 3000)
      }).catch(error => {

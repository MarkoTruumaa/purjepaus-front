    ...
    <div class="container">
      ...
      <div class="row justify-content-center">
        <div class="col col-4">
          ...
          <div class="input-group mb-3">
            ...
            <select v-model="selectedCountyName" class="form-select" aria-label="Default select example">
          ...
          <div class="input-group mb-3">
            ...
            <input v-model="harbourDetailedInfo.spots" type="number" class="form-control"
                   aria-describedby="basic-addon1">
          </div>
        </div>
        <div class="col col-4">
      ...
      <div>
        <div class="container">
          <div class="row">
            <div class="col">
              <button @click="updateHarbourInfoRequest" type="button" class="btn btn-success">Muuda sadamat</button>
            </div>
            <div class="col">
              <button @click="this.$router.go(-1)" type="button" class="btn btn-primary">Tagasi sadamate juurde</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    sendUpdateHarbourInfoRequest() {
      this.$http.put("/harbour", this.harbourDetailedInfo, {
            params: {
              harbourId: this.id
            },
          }
      ).then(response => {
        this.successMessage = HARBOUR_LOCATION_UPDATED.replace('?', this.harbourDetailedInfo.harbourName)
      }).catch(error => {
        router.push({name: 'errorRoute'})
      })
    },
